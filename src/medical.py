...
class MedicalPlayer(gym.Env):
    ...
    def __init__(self, directory=None, viz=False, task=False, files_list=None,
                 ...
                 physics=None, shaping_gamma=0.9, shaping_weight=0.0):
        """
        ...
        self.logger = logger
        # --- physics prior (optional; disabled when physics is None) ----------
        # `physics` is a physics.PhysicsProvider. `shaping_weight`=0 keeps the
        # environment byte-for-byte identical to the original (safe default).
    ...
    def _calc_reward(self, current_loc, next_loc, agent):
        ...
        # --- Hamilton-Jacobi potential-based reward shaping -------------------
        # r' = r + gamma * Phi(s') - Phi(s),  with Phi(s) = -V(s) the negative
        # geodesic navigation potential. By Ng, Harada & Russell (1999) this is
        # guaranteed to leave the optimal policy unchanged while giving a dense,
        # anatomy-aware signal. This is the *correct* replacement for the PDF's
        # non-invariant  R = -dd - lambda||grad V|| + gamma C.
        if self.physics is not None and self.shaping_weight != 0.0:
            vol = self._image[agent].data
            seed = self._target_loc[agent]
            iid = self.filename[agent] if hasattr(self, "filename") else "img"
            phi_s = self.physics.shaping_potential(vol, seed, current_loc, iid)
            phi_sp = self.physics.shaping_potential(vol, seed, next_loc, iid)
            reward = reward + self.shaping_weight * (
                self.shaping_gamma * phi_sp - phi_s)
        return reward
